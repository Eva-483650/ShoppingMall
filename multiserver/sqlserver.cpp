...
// 开始事务
bool SQLServer::beginTransaction() {
    if (!db.isOpen()) {
        qDebug() << "数据库未连接，无法开始事务";
        return false;
    }
...
// 提交事务
bool SQLServer::commitTransaction() {
    if (!db.isOpen()) {
        qDebug() << "数据库未连接，无法提交事务";
        return false;
    }
...
// 回滚事务
bool SQLServer::rollbackTransaction() {
    if (!db.isOpen()) {
        qDebug() << "数据库未连接，无法回滚事务";
        return false;
    }
    ...
    if (db.rollback()) {
        ...
        transactionActive = false; // 强制重置状态
        return false;
    }
}

// 检查事务是否活动
bool SQLServer::isTransactionActive() {
...
bool SQLServer::selectSth(QString table, QJsonObject obj, QJsonArray& result) {
    ...
    if (obj.contains("want")) {
        ...
        // 判断是否需要DISTINCT
        if (obj.contains("isDistinct") && obj.value("isDistinct").toString().toLower() == "true") {
            isDistinct = true;
        }
        // 判断是否有查询条件
        ...
        // 判断是否有排序条件
        if (obj.contains("orderBy")) {
            orderBy = obj.value("orderBy").toString();
        }
        ...
        // 执行SQL查询
        bool success = q.exec(content);
        if (!success) {
            qDebug() << "SQL执行失败:" << content;
...
bool SQLServer::insertSth(QString table, QJsonObject obj) {
    ...
    if (SQLhead.isEmpty()) {
        qDebug() << QString("表%1为空或不存在！").arg(table);
        return false;
    }
    else {
        // 拼接SQL语句
        ...
        QSqlQuery q(db);
        bool success = q.exec(insertTableContent);
        if (!success) {
