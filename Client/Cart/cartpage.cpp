...
QListWidget::item:hover {
    background: rgba(248, 245, 252, 255);
    border: none;
}

QListWidget::item:selected {
    background: rgba(245, 240, 250, 255);
    border: none;
}
...
QListWidget::item:focus {
    ...
    ui->list_item->setStyleSheet(listStyle);

    // 禁用列表项的选择功能，因为我们使用RadioButton来控制选中状态
    ui->list_item->setSelectionMode(QAbstractItemView::NoSelection);
...
void CartPage::addToCart(Product* product)
{
    ...
    // 设置列表项的固定大小
    lwItem->setSizeHint(QSize(0, 120));
...
void CartPage::commitQuantityToServer(int proId, int newQty)
{
    ...
    if (ok) {
        ...
        ci->setQuantity(rollback); // 使用新的方法名

        ci->updateTotalLabel(QString("￥%1").arg(rollback * ci->unitPrice()));
